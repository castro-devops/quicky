type FieldErrorMap<T> = Partial<
  Record<keyof T, new (args: { message: string }) => Error>
> & {
...
export function requiredFields<T>(
  ...
  for (const field in requiredFields) {
    ...
    if (value === undefined || value === null || value === '') {
      const ErrorClass =
        fieldErrorMap[field as keyof T] ?? fieldErrorMap.general;

      if (!ErrorClass) {
        throw new Error(
          `Missing required field "${field}" but no error class provided`,
        );
      }
